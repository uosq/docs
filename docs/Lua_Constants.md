...
You can always use the `Name.Value` or just the `Value` (See [Examples](#examples))
...
``` lua title="E_ClientFrameStage example"
local function OnFrameStageNotify(stage)
    if stage == FRAME_RENDER_START then
        local localPlayer = entities.GetLocalPlayer()
        if localPlayer == nil then return end
        localPlayer:SetVAngles(Vector3(0, 0, 0))
    end
end
```
``` lua title="E_UserCmd example 2"
local function OnCreateMove(cmd)
    --- You can see we use E_UserCmd.IN_ATTACK instead of only IN_ATTACK here
    --- There is no difference, both work the same way
    if (cmd.buttons & E_UserCmd.IN_ATTACK) ~= 0 then
        print("Pressing M1")
    end
end
